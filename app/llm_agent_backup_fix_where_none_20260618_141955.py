...
def build_where_sql(where_clauses):
    ...
    for clause in where_clauses or []:
        ...
        # Some helper functions return (sql_condition, label).
        # WHERE should only use the SQL condition.
        if isinstance(clause, tuple):
            clause = clause[0]

        # If a list accidentally gets appended, flatten it safely.
        if isinstance(clause, list):
            for item in clause:
                ...
                if item_text:
                    clean_clauses.append(item_text)
        ...
        if clause_text:
            clean_clauses.append(clause_text)
