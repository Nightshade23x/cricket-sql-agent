...
""".strip()
    ...
    venue_case = venue_group_sql("m.venue")

    venue_performance_sql = f"""
SELECT TOP 10
    {venue_case} AS venue,
    SUM(d.runs_off_bat) AS runs,
    COUNT(CASE WHEN COALESCE(d.wides, 0) = 0 THEN 1 END) AS balls_faced,
    ...
    ROUND(
        SUM(d.runs_off_bat) * 100.0 /
        NULLIF(COUNT(CASE WHEN COALESCE(d.wides, 0) = 0 THEN 1 END), 0),
        2
    ) AS strike_rate
FROM deliveries d
JOIN matches m
...
def venue_group_sql(venue_column="m.venue"):
    return f"""
CASE
    WHEN {venue_column} LIKE '%Chinnaswamy%' THEN 'M Chinnaswamy Stadium'
    WHEN {venue_column} LIKE '%Chidambaram%' OR {venue_column} LIKE '%Chepauk%' THEN 'MA Chidambaram Stadium, Chepauk'
    WHEN {venue_column} LIKE '%Wankhede%' THEN 'Wankhede Stadium'
    WHEN {venue_column} LIKE '%Eden Gardens%' THEN 'Eden Gardens'
    WHEN {venue_column} LIKE '%Narendra Modi%' OR {venue_column} LIKE '%Motera%' OR {venue_column} LIKE '%Sardar Patel%' THEN 'Narendra Modi Stadium, Ahmedabad'
    WHEN {venue_column} LIKE '%Rajiv Gandhi%' OR {venue_column} LIKE '%Uppal%' THEN 'Rajiv Gandhi International Stadium'
    WHEN {venue_column} LIKE '%Feroz Shah Kotla%' OR {venue_column} LIKE '%Arun Jaitley%' THEN 'Arun Jaitley Stadium'
    WHEN {venue_column} LIKE '%Dubai International%' THEN 'Dubai International Cricket Stadium'
    WHEN {venue_column} LIKE '%Sharjah%' THEN 'Sharjah Cricket Stadium'
    WHEN {venue_column} LIKE '%Zayed%' OR {venue_column} LIKE '%Abu Dhabi%' THEN 'Sheikh Zayed Stadium'
    ELSE {venue_column}
END
""".strip()
